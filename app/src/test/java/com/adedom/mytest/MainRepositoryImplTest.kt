...
class FakeMainLocalDataSourceImplTest : MainLocalDataSource {
...
class MainRepositoryImplTest {
    ...
    private val mainRemoteDataSource: MainRemoteDataSource = mockk(relaxed = true)

    @Test
    fun fetchMainList_returnSuccess() {
        mainLocalDataSource = FakeMainLocalDataSourceImplTest()
        repository = MainRepositoryImpl(mainLocalDataSource, mainRemoteDataSource)
        val mainList = listOf(
            MainModel(
                id = 1,
        ...
        every { mainRemoteDataSource.fetchMainList() } returns mainList

        val result = repository.fetchMainList()

        val mainListResult = listOf(
            MainModel(
                id = 1,
                image = "https://www.adslthailand.com/uploads/moxie/2019/android-q.png",
                title = "What is Lorem Ipsum?",
        ...
        assertEquals(mainListResult, result)
        assertEquals(mainListResult, mainLocalDataSource.getMainData())
    }
    ...
    fun fetchMainList_returnError() {
        ...
        every { mainRemoteDataSource.fetchMainList() } throws exception

        val result = repository.fetchMainList()

        assertEquals(emptyList<MainModel>(), result)
        assertEquals(emptyList<MainModel>(), mainLocalDataSource.getMainData())
    }
}